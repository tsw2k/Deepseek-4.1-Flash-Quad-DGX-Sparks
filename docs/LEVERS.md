...
| L4 | b12x MXFP8 dense kernel | `EXTRA_PY_DIR=/var/tmp/dsv41-b12x` plus `VLLM_DISABLED_KERNELS=FlashInferCutedslMxfp8LinearKernel,FlashInferCutlassMxfp8LinearKernel,MarlinMxfp8LinearKernel` in `LEVER_ENV` | MiaAI's profile: dense FP8 projections 52 ms on Triton, 50 on CUTLASS, 17 on b12x per step. bot-lab-21 accepted the same route in vLLM | MiaAI; bot-lab-21 | **queued** 2026-09-22. `b12x==1.3.0` is pure Python and its pinned dependencies are already in the image at the same versions, so it is installed with `pip --target` on each node and mounted (no image layer, nothing to ship). B12x is fourth in the MXFP8 list, hence the three disabled kernels; the BMM path that carries 0xTank's compact projection only ever considers DeepGemm and Emulation, so this lever does not touch it. If `is_supported()` fails on GB10 the engine falls through to Emulation silently, which is why `bench/run.sh` now records the chosen kernels |
