...
| L7 | sysctls | `vm.min_free_kbytes=1048576`, `vm.watermark_scale_factor=200` | earlier reclaim on the unified pool | Tech2Wild | **accepted on memory grounds** 2026-09-16: MemFree 1-5 GiB to 7-9 GiB per node, prefill and needle flat within 1-4 %; decode not measurable that day (thermals and acceptance drifted). Persisted as `ops/99-dsv41-sysctl.conf`; `launch/node.sh`'s boot floor moved 100 to 95 GiB because the reserve is subtracted from MemAvailable ([results](../results/2026-09-16-L7-sysctls/NOTES.md)) |
