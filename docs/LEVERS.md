...
| L3 | K5 instead of K3 | `SPEC_K=5`, `MAXLEN=300000` | Tech2Wild's boot 10 shape; K3 was 0xTank's choice for 600K. Content-dependent: acceptance near 6 on code and counting, near 2 on prose | Tech2Wild | **meets the rule** 2026-09-19 (night A/B/A): C1 per-stream +10.9 %, C5 +7.6 %, C6 +4.4 %, prefill -0.6 %, acceptance 3.67 vs 2.96; costs the 600K request length, KV pool unchanged. **Accepted** 2026-09-19 (300K is enough for the users); in `cluster.env.example` and applied at the night relaunch of 2026-09-20 ([results](../results/2026-09-19-night-L3-L5/NOTES.md)) |
| L4 | b12x MXFP8 dense kernel | `EXTRA_PY_DIR=/var/tmp/dsv41-b12x` plus `VLLM_DISABLED_KERNELS=FlashInferCutedslMxfp8LinearKernel,FlashInferCutlassMxfp8LinearKernel,MarlinMxfp8LinearKernel` in `LEVER_ENV` | MiaAI's profile: dense FP8 projections 52 ms on Triton, 50 on CUTLASS, 17 on b12x per step. bot-lab-21 accepted the same route in vLLM | MiaAI; bot-lab-21 | **rejected** 2026-09-23 (night A/B/A): the kernel was selected and the output was corrupted, stray foreign-vocabulary tokens inside otherwise coherent text, 5/10 gates failed ([results](../results/2026-09-23-night-L4-b12x/NOTES.md)). Setup kept for a later b12x release: `b12x==1.3.0` is pure Python, installed with `pip --target` and mounted via `EXTRA_PY_DIR`; B12x is fourth in the MXFP8 list, hence the three disabled kernels; the compact-projection BMM path never considers it |
| L5 | minimal patch set | `PATCH_SET=minimal` | keeps the tree's own changes to the indexer, `weight_utils` and `sparse_swa` instead of rolling them back | this repo | **run** 2026-09-19 (night A/B/A): gates 10/10, probe on the floor, speed within the night's spread, KV pool -6 % (1.52M vs 1.60-1.63M tokens). Not the default; it proves the recipe's own diffs sound ([results](../results/2026-09-19-night-L3-L5/NOTES.md)) |
| L6 | page-cache flusher on | `glm53-flusher.service` running during serving | the GLM stack needed it at load; during serving it evicts Engram pages every 60 s | mtxc GLM ops | **rejected** 2026-09-24 (night A/B/A): C6 -5.9 %, C5 -5.6 %, prefill at 46,810 -5.4 %, needle 3.3 % slower, C1 unchanged. Dropping the page cache sends shared Engram reads back to NVMe, as L7 did ([results](../results/2026-09-24-night-L6-flusher/NOTES.md)) |
